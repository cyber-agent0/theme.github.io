...
// AJAX handler for getting anime details
function donghua_hub_get_anime_details() {
    ...
    if ($post && $post->post_type === 'anime') {
        ...
        $data = array(
            ...
            'description' => get_the_excerpt($post_id) ?: wp_trim_words(get_the_content(null, false, $post_id), 50),
...
// Track post views
function donghua_hub_track_post_views($post_id) {
    if (!is_single()) return;
    if (empty($post_id)) {
        global $post;
        $post_id = $post->ID;
...
add_action('wp_head', function() {
    if (is_single()) {
        global $post;
        donghua_hub_track_post_views($post->ID);
    }
});
?>